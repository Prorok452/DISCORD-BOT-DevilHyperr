	...
	constructor(client) {
		super(client, {
			...
			args: [
				{
					key: 'command',
					prompt: 'Którą komendę chciałbyś załadować?',
					validate: val => new Promise(resolve => {
						...
						if(this.client.registry.findCommands(val).length > 0) {
							return resolve('Ta komenda jest już zarejestrowana');
						}
	...
	async run(msg, args) {
		...
		if(this.client.shard) {
			try {
				...
				await msg.reply(`Załadowałem \`${command.name}\` tę komendę, ale nie udało mi się załadować innych elementów.`);
				return null;
			}
		}

		await msg.reply(`Załadowałem \`${command.name}\` command${this.client.shard ? ' i wszystkie elementy' : ''}.`);
		return null;
	}
};
