...
class Command {
	...
	hasPermission(message, ownerOverride = true) {
		...
		if(this.ownerOnly && (ownerOverride || !this.client.isOwner(message.author))) {
			return `\`${this.name}\` Komenda może być użyta tylko przez właściciela.`;
		}
		...
		if(message.channel.type === 'text' && this.userPermissions) {
			...
			if(missing.length > 0) {
				if(missing.length === 1) {
					return `Ta komenda \`${this.name}\` wymaga abyś posiadał te permisje "${permissions[missing[0]]}"`;
				}
				return oneLine`
					Ta komenda \`${this.name}\` wymaga abyś posiadał te permisje:
	...
	onBlock(message, reason, data) {
		switch(reason) {
			case 'guildOnly':
				return message.reply(` \`${this.name}\` ta komenda musi być użyta na kanale serwerowym.`);
			case 'nsfw':
				return message.reply(` \`${this.name}\` ta komenda może być używana tylko na kanałach NSFW`);
			case 'permission': {
				if(data.response) return message.reply(data.response);
				return message.reply(`Nie masz permisji żeby użyć tej \`${this.name}\` komendy.`);
			}
			case 'clientPermissions': {
				if(data.missing.length === 1) {
					return message.reply(
						`Potrzebuje tej "${permissions[data.missing[0]]}" permisji, żeby ta komenda zadziałała \`${this.name}\` `
					);
				}
				return message.reply(oneLine`
					Potrzebuje tej permisji, żeby ta komenda \`${this.name}\` zadziałała:
					${data.missing.map(perm => permissions[perm]).join(', ')}
				`);
			}
			case 'throttling': {
				return message.reply(
					`Nie możesz użyć tej komendy \`${this.name}\` przez następne ${data.remaining.toFixed(1)} sekundy.`
				);
			}
			default:
				return null;
	...
	onError(err, message, args, fromPattern, result) { // eslint-disable-line no-unused-vars
		...
		return message.reply(stripIndents`
			Wystąpił error podczas próby wywołania komendy: \`${err.name}: ${err.message}\`
			You shouldn't ever receive an error like this.
			Skontaktuj się z ${ownerList || 'właścicielem'}${invite ? ` na tym serwerze: ${invite}` : '.'}
		`);
	}
