	...
	async obtain(msg, val, promptLimit = Infinity) {
		...
		while(!valid || typeof valid === 'string') {
			...
			prompts.push(await msg.reply(stripIndents`
				${empty ? this.prompt : valid ? valid : `Wprowadziłeś zły ${this.label}. Spróbuj ponownie.`}
				${oneLine`
					Wpisz \`cancel\`, żeby zatrzymać komendę
					${wait ? `Komenda zostanie automatycznie zatrzymana za ${this.wait} sekund.` : ''}
				`}
			`));
		...
		while(true) { // eslint-disable-line no-constant-condition
			...
			while(!valid || typeof valid === 'string') {
				...
				if(val) {
					...
					prompts.push(await msg.reply(stripIndents`
						${valid ? valid : oneLine`
							Wprowadziłeś zły ${this.label},
							"${escaped.length < 1850 ? escaped : '[za długie, żeby pokazać]'}".
							Spróbuj ponownie.
						`}
						${oneLine`
							Wpisz \`cancel\`, żeby zatrzymać komendę
